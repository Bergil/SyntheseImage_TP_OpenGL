...
void UniformMat4::update( )
{
    ...
    char tmp[1024];
    sprintf(tmp, "% 10.3f % 10.3f % 10.3f % 10.3f \n% 10.3f % 10.3f % 10.3f % 10.3f \n% 10.3f % 10.3f % 10.3f % 10.3f \n% 10.3f % 10.3f % 10.3f % 10.3f", 
...
GLProgramUniforms::GLProgramUniforms( GLProgram *_program ) : names(), values(), program(GLProgram::null()), program_changes(0)
{
    ...
    {   // samplers
        unsigned int n= program->samplerCount();
        for(unsigned int i= 0; i < n; i++)
        {
            ProgramSampler sampler= program->sampler(i);
            for(int id= 0; id < sampler.size; id++)
            {
                names.push_back( program->samplerName(sampler) );
                values.push_back( UniformValue::create(sampler, id) );
            }
        }
        printf("%lu samplers\n", values.size());
    }
