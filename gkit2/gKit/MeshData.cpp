...
int buildIndex( const std::vector<Vec3>& points, MeshData& mesh )
{
    ...
    unsigned int n= indices.size() / 3u;
    ...
    for(;;)
    {
        ...
        if(line[0] == 'K')
        {
            ...
            if(sscanf(line, "Ka %f %f %f", &r, &g, &b) == 3) 
            {
                ...
                material->ambient_color= Vec4(v.x, v.y, v.z, 1);
                material->ka= k;
            }
            else if(sscanf(line, "Kd %f %f %f", &r, &g, &b) == 3) 
            {
                ...
                material->diffuse_color= Vec4(v.x, v.y, v.z, 1);
                material->kd= k;
            }
            else if(sscanf(line, "Ks %f %f %f", &r, &g, &b) == 3) 
            {
                ...
                material->specular_color= Vec4(v.x, v.y, v.z, 1);
                material->ks= k;
            }
        }
        ...
        else if(line[0] == 'm')
        {
            if(sscanf(line, "map_Ka %[^\r\n]", tmp) == 1)
                ...
                material->normal_texture= IOFileSystem::pathname( filename ) + tmp;
