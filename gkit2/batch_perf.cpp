...
public:
        ...
        gk::App()
    {
        ...
        settings.setGLVersion(4,4);     // version 4.4, utilisation de glBufferStorage + glMultidrawIndirect
    ...
    int init( )
    {
        ...
        // alterne entre 2 textures
        gk::Image *image= gk::ImageIO::readImage("bigguy_ambient.png");
        if(image == NULL)
            return -1;
        m_textures[0]= gk::createTexture2D(GL_TEXTURE0, image);
        m_textures[1]= gk::createTexture2D(GL_TEXTURE0, image);
    ...
    int draw( )
    {
        ...
        if(m_multi_draw)
        {
            ...
            m_program[program_id]->sampler("color")= 0;
            
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, m_textures[0]->name);
        ...
        else
        {
            for(int draw= 0; draw < m_draw_count; draw++)
            {
                ...
                if(current_program != program_id)
                {
                    ...
                    m_program[current_program]->sampler("color")= 0;
                }
                ...
                if(current_mesh != mesh_id)
                {
                    ...
                    if(m_static_vao)
                    {
                        if(current_vao != m_mesh[current_mesh]->vao->name)
                        {
                ...
                if(current_texture != texture_id)
                {
                    ...
                    m_program[current_program]->sampler("color")= 0;
                    glActiveTexture(GL_TEXTURE0);
                    glBindTexture(GL_TEXTURE_2D, m_textures[current_texture]->name);
                }
        ...
        long int cpu_average= 0;
        for(unsigned int i= 0; i < m_cpu_graph.size(); i++)
            cpu_average+= (long int) m_cpu_graph[i];
        cpu_average/= (long int) m_cpu_graph.size();
        ...
        m_widgets.beginGroup(nv::GroupFlags_GrowDownFromLeft);
            ...
            sprintf(tmp, "cpu time % 6dus [% 6ldus:%d]", cpu_time / 1000, cpu_average / 1000, (int) m_cpu_graph.size());
            m_widgets.doLabel(nv::Rect(), tmp);
