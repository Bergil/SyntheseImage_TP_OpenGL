...
public:
    ...
    int init( )
    {
		...
		m_projection_depth = gk::Perspective(70.0, 1.0, 0.1, 1000.0);
    ...
    int draw( )
    {
        ...
        shadowlight.light_position = gk::Point(100*(std::cos(time))+80, 10.0, 75.0);
        shadowlight.light_color = gk::Color(1.0, 1.0, 1.0);
        
		VP =  m_projection * gk::LookAt(m_camera_position, m_camera_position + m_view_direction, gk::Vector(0.0, 1.0, 0.0));
		VP_Light = m_projection_depth * gk::LookAt(shadowlight.light_position, shadowlight.light_position+gk::Vector(0.0,-0.1,-1.0), gk::Vector(0.0, 1.0, 0.0));
